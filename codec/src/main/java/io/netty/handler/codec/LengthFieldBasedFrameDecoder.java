...
public class LengthFieldBasedFrameDecoder extends ByteToMessageDecoder {
    ...
    /**
     * 如果为true，则表示读取到长度域，TA的值的超过maxFrameLength，
     * 就抛出一个 TooLongFrameException，而为false表示只有当真正读取完
     * 长度域的值表示的字节之后，才会抛出TooLongFrameException，
    ...
    private void exceededFrameLength(ByteBuf in, long frameLength) {
        ...
        if (discard < 0) {
            ...
            //当前可读字节已经到达frameLength，直接跳过frameLength个字节，丢弃之后后面也可能就是一个合法的数据包
    ...
    private void failIfNecessary(boolean firstDetectionOfTooLongFrame) {
        if (bytesToDiscard == 0) {//不需要再丢弃后面的未读字节，就开始重置丢弃状态
            ...
            //如果设置了快速失败，并且是第一次检测到大包错误，抛出异常，让handler去处理
